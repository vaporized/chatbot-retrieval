...
def memn2n_model_3d_layerwise(
    hparams,
    mode,
    context,
    utterance,
    targets):
    """the 3d memn2n model in layerwise mode.
    ...
    with tf.variable_scope('memn2n_model'):
        ...
        #look up context in embedding matrix A, (batch_size, memory_size, sentence_len, embedding_size) and C
        embedded_context_A = tf.nn.embedding_lookup(A, context, name='embedded_context_A')
        embedded_context_C = tf.nn.embedding_lookup(C, context, name='embedded_context_C')
        #reduce embedding vectors to (batch_size, memory_size, embedding_size)
